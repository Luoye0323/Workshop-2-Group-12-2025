...
def create_precise_template(output_file: str):
    ...
    # General Info Table
    # Rows: 2, Cols: 5
    left, top, width, height = Inches(0.5), Inches(0.7), Inches(10.69), Inches(0.9)
    ...
    logo_top = Inches(1.07)
    # Dimensions
    logo_width = Inches(0.58)
    logo_height = Inches(0.48)

    # Try to find the logo
    # Use PNG with transparency preserved by sips conversion
    possible_logo_paths = [
        "public/ipetro-vertical.png",  # Transparent PNG (best option)
        os.path.join(
            os.path.dirname(__file__), "public", "ipetro_vertical.png"
        ),
        "public/ipetro-logo-transparent.png",  # Fallback
        os.path.join(os.path.dirname(__file__), "public", "ipetro-logo-transparent.png"),
    ]
